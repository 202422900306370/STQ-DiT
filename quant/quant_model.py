...
class QuantModel(nn.Module):
    ...
    # filepath: f:\2025.6.9 diffusion for SR\baseline\PTQ4DiT\PTQ4DiT-main\quant\quant_model.py
    def quant_module_refactor(self, module: nn.Module, weight_quant_params: dict = {}, act_quant_params: dict = {}, prefix=""):
        for name, child_module in module.named_children():
            full_name = f"{prefix}.{name}" if prefix else name
            if isinstance(child_module, (nn.Conv2d, nn.Conv1d, nn.Linear)):
                # 传递真实层名
                setattr(module, name, QuantModule(
                    child_module, weight_quant_params, act_quant_params, name=full_name))
            elif isinstance(child_module, StraightThrough):
                continue
            else:
                self.quant_module_refactor(child_module, weight_quant_params, act_quant_params, prefix=full_name)
