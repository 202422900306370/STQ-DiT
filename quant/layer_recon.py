...
def layer_reconstruction(model: QuantModel, layer: QuantModule, cali_data: torch.Tensor,
    ...
    # Set up optimizer:
    alpha_params, bounds_params = [], []
    if layer.weight_quantizer.lwc:
        alpha_params = [layer.weight_quantizer.alpha]
        bounds_params = [layer.weight_quantizer.upbound_factor, layer.weight_quantizer.lowbound_factor]
    else:
        alpha_params = [layer.weight_quantizer.alpha]

    optimizer_w = torch.optim.Adam(
        [
            {'params': alpha_params, 'lr': 8e-4},
            {'params': bounds_params, 'lr': 8e-5},
        ]
    )
    # scheduler_w = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_w, T_max=iters, eta_min=0.)
    ...
    for k in range(num_split):
        ...
        for i in range(iters // num_split):
            ...
            # if i % 500 == 0:  # 防刷屏，可调整间隔或去掉条件
            #     for name, module in layer.named_modules():
            #         if isinstance(module, QuantModule) and getattr(module.weight_quantizer, "lwc", False):
            #             wq = module.weight_quantizer
            #             print(f"[k={k} i={i}] {name} upbound grad:", wq.upbound_factor.grad)
            #             print(f"[k={k} i={i}] {name} lowbound grad:", wq.lowbound_factor.grad)
            #             print(f"[k={k} i={i}] {name} alpha grad:", wq.alpha.grad)
            #             break  # 仅查看首个量化模块，想看全部去掉此行
